...
| 팀원 | 역할 | 세부 내용 |
| --- | --- | --- |
| **엄현식** | GRU 딥러닝 모델 구축<br>데이터 크롤링<br>데이터 전처리 | - 모델 학습 및 테스트용 데이터 수집<br>- 음원 목록 및 차트, 음원 리뷰 크롤링<br>- 모델 학습 및 테스트용 데이터 전처리<br>- 음원 데이터 전처리 |
| **강규현** | LSTM 딥러닝 모델 구축<br>데이터 전처리<br>Flask 서버 구축 | - 모델 학습용 데이터 전처리 (약 10만개) |
| **김수빈** | 전반적 기획 분석<br>문서 작업<br>PPT 제작<br>서브 서버 대비 (Node.js)<br>웹페이지 디자인 | - 기획서, 개요서, 요구사항 정의서, 화면 설계서 작성 |
| **박승현** | 프론트엔드 개발<br>백엔드 개발 | - AJAX, JSON, jQuery 활용<br>- 회원 유효성 검사 구현<br>- MVC 패턴 적용<br>- 페이지네이션 구현 |
| **최재민** | 데이터 크롤링<br>DB 관리 및 테이블 설계<br>백엔드 데이터 출력 | - MySQL 연결 및 반영<br>- 음악 차트, 뉴스 수집<br>- 지속적인 테이블 수정<br>- 데이터 무결성 유지<br>- 데이터 출력 관련 기능 개발 |
