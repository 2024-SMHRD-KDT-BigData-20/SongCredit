...
## 주요기능
#### 1. 음원 댓글 감정 분석: 사용자 댓글을 통해 음원의 감정을 분석하고, 이를 바탕으로 투자 추천을 제공.
#### 2. 음원 동향 분석: 최근 음원의 인기도 및 동향을 분석하여 제공.
#### 3. 맞춤형 추천: 사용자 취향과 감정 분석 결과를 기반으로 맞춤형 음악저작권 투자 상품 추천.
#### 4. 데이터 시각화: 음원 동향 및 감정 분석 결과를 시각화하여 사용자에게 제공.
...
![Matplotlib](https://img.shields.io/badge/Matplotlib-FF5733?style=flat&logo=python&logoColor=white)
...
## 테이블 구성
![image](https://github.com/2024-SMHRD-KDT-BigData-20/SongCredit/assets/160554303/2c65d88a-1f80-46ed-b2aa-3e11bfabfcef)


## 화면구성
#### 1. 홈 화면: 최신 음원 동향 및 인기 음원 표시
#### 2. 검색 화면: 사용자가 원하는 음원 검색 및 댓글 조회
#### 3. 감정 분석 결과 화면: 음원 댓글의 감정 분석 결과 제공
#### 4. 추천 화면: 맞춤형 음악저작권 투자 상품 추천
...
| 팀원 | 역할 | 세부 내용 |
| --- | --- | --- |
| **엄현식** | GRU 모델 구축<br> 데이터 크롤링<br> 데이터 전처리 | - 모델 학습 및 테스트용 데이터 수집<br>- 음원 목록 및 차트, 음원 리뷰 크롤링<br>- 모델 학습 및 테스트용 데이터 전처리<br>- 음원 데이터 전처리 |
| **강규현** | LSTM 모델 구축<br> 데이터 전처리<br> Flask 서버 | - 모델 학습용 데이터 전처리 (약 10만개) |
| **김수빈** | 기획 분석<br> 문서 작업<br> PPT 제작<br> 서브 서버(Node.js)<br> 웹 디자인 | - 기획서, 개요서, 요구사항 정의서, 화면 설계서 작성 |
| **박승현** | 프론트엔드 개발<br> 백엔드 개발 | - AJAX, JSON, jQuery 활용<br>- 회원 유효성 검사 구현<br>- MVC 패턴 적용<br>- 페이지네이션 구현 |
| **최재민** | 데이터 크롤링<br> DB 관리 및 설계<br> 백엔드 출력 | - MySQL 연결 및 반영<br>- 음악 차트, 뉴스 수집<br>- 지속적인 테이블 수정<br>- 데이터 무결성 유지<br>- 데이터 출력 관련 기능 개발 |
